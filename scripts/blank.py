...
def generate_molecule_worker(args: ty.Tuple[ty.Any, int, float]) -> ty.Tuple[int, ty.Any]:
    # Worker function to generate a single molecule and return its index and molecule object
    predictor_model, pred_limit, index = args
    mol = gen_molecule(predictor_model)
    while mol.pred_value < pred_limit:
        mol = gen_molecule(predictor_model)
    return index, mol
...
def main() -> None:
    ...
    # Generate molecules using multiprocessing
    with multiprocessing.Pool() as pool:
        generated_molecules = []
        num_mols = 0
        index = 0
        while num_mols < num_wanted:
            # Generate one molecule per process
            results = pool.map(generate_molecule_worker, [(predictor_model, pred_limit, index)])
            for result_index, mol in results:
                if mol.pred_value >= pred_limit:
                    generated_molecules.append((result_index + 1, mol.SMILES, mol.pred_value))
                    num_mols += 1
                index += 1

    # Save generated molecules to output file
    with open(output_file, 'w') as file:
        file.write("ID\tSMILES\tpred_value\n")
        for mol_info in generated_molecules:
            file.write(f"{mol_info[0]}\t{mol_info[1]}\t{mol_info[2]}\n")
